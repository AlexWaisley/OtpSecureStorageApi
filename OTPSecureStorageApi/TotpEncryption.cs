...
public static class TotpEncryption
{
    ...
    public static string GenerateCode(this Totp totp)
    {
        var date = DateTimeOffset.UtcNow.ToUnixTimeSeconds();
        var counter = date / 30;

        var res = (int)(HotpCalc(totp.SecretKey, counter) % Math.Pow(10, totp.DigitsCount));
        return res.ToString($"D{totp.DigitsCount}");
    }
}